...
#define NO_ROWS 4
const byte r[]={9, 10, 11, 8};

#define NO_COLS 6
const byte c[]={2, 3, 4, 5, 6, 7};

void setup() {
  for(int i=0; i<NO_COLS; i++)  pinMode(c[i], INPUT_PULLUP);
  for(int i=0; i<NO_ROWS; i++)  pinMode(r[i], INPUT);
  pinMode(13, OUTPUT);
  digitalWrite(13, LOW);
  Serial.begin(9600);
...
void loop() {
  for(int i=0; i<NO_BYTES; i++)  b[i] = i * 0x40;
  ...
  for(int i=0; i<NO_BYTES; i++)  Serial.write(b[i]);
}

void scan() {
  digitalWrite(13, HIGH); // toggle LED to time the scan
  pressed = false;
  for(int i=0; i<NO_BYTES; i++)  b[i] |= getRow(r[i]);
  digitalWrite(13, LOW);
}  
...
byte getRow(int pin) {
  ...
  byte row = 0;
  for(int i=0; i<NO_COLS; i++)  row |= pinState(c[i]) << i;
  pinMode(pin, INPUT);
  return row;
}
